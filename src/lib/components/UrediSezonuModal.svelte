<script lang="ts">
	...
	async function handleDodajKaveze() {
		...
		try {
			...
			dodajError = err instanceof Error ? err.message : 'Greška pri dodavanju kaveza';
		} finally {
			dodajLoading = false;
		}
	}
	...
	async function handleObrisiKavez(kavez: Kavez) {
		...
		try {
			...
			alert(err instanceof Error ? err.message : 'Greška pri brisanju kaveza');
		} finally {
	...
	async function handleObrisiSezonu() {
		...
		try {
			...
			brisanjeError = err instanceof Error ? err.message : 'Greška pri brisanju sezone';
	...
	<div class="card w-full max-w-md p-6 space-y-4 max-h-[90vh] flex flex-col">
		...
		<div class="flex gap-1 shrink-0">
			...
			<button
				...
				on:click={() => (tab = 'kavezi')}
			>
				🏠 Kavezi <span class="badge variant-soft ml-1 text-xs">{trenutniKavezi.length}</span>
			</button>
		</div>
			...
			<div class="space-y-4">
				...
				<div class="space-y-1 max-h-48 overflow-y-auto">
					...
					{#if trenutniKavezi.length === 0}
						<p class="text-xs text-surface-400 text-center py-3">Nema kaveza u ovoj sezoni.</p>
					{/if}
				</div>

				<!-- Dodaj kaveze -->
				<div class="card p-3 space-y-3 variant-soft">
					<p class="text-sm font-semibold">Dodaj kaveze</p>
					<div class="flex items-end gap-2">
						<label class="label flex-1">
							<span class="text-xs">Broj kaveza</span>
							<input
								class="input"
								type="number"
						...
						{#if sekcije.length > 0}
							<label class="label flex-1">
								<span class="text-xs">Sekcija</span>
					...
					>
						...
						+ Dodaj {dodajBroj} {dodajBroj === 1 ? 'kavez' : 'kaveza'}
					</button>
				</div>

				<!-- Danger zone: brisanje sezone -->
				<div class="border border-error-300 dark:border-error-700 rounded-lg p-3 space-y-2">
					<p class="text-sm font-semibold text-error-600 dark:text-error-400">Obriši sezonu</p>
					{#if !mozeBrisati}
						<p class="text-xs text-surface-400">
							Sezona se može obrisati samo ako su svi kavezi prazni (bez aktivnih ili završenih ciklusa).
						</p>
					{:else}
						<p class="text-xs text-surface-400">
							Svi kavezi su prazni. Brisanje će ukloniti sezonu i sve kaveze trajno.
						</p>
					{/if}
					{#if brisanjeError}
